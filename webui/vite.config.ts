...
export default defineConfig(({ mode }) => {
  ...
  const target = env.CAPYBOT_API_URL ?? "http://127.0.0.1:8765";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../capybot/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
      rollupOptions: {
        output: {
    ...
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the Capybot ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the Capybot gateway;
